...
class Calculator(QWidget):
    ...
    def createButtons(self):
        ...
        for btn in self.buttons.values():
            if btn.text() in {'/', 'X', '-', '+', '='}:
                btn.setStyleSheet('background-color: orange; color: white; font-size: 20px; border-radius: 40px;')
            else:
                btn.setStyleSheet('background-color: gray; color: white; font-size: 20px; border-radius: 40px;')
